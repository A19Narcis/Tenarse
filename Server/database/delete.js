...
const deletePost = async function (idPost, username, callback) {
    await readDB.getPublicacio(idPost, (deletedPost) => {
        if (deletedPost.tipus == 'image') {
            const path_inicial = deletedPost.url_img;
            const path_final = path_inicial.replace('http://212.227.40.235/Tenarse/Server/', ""); //localhost:3000 -> 212.227.40.235
            ...
            const path_final = path_inicial.replace('http://212.227.40.235/Tenarse/Server/', "");
            fs.unlink(path_final, (err) => {
                if (err) throw err;
            });
        }
